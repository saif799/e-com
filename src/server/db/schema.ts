import { pgTable, index, foreignKey, text, timestamp, unique, integer } from "drizzle-orm/pg-core"
...
(table) => {
	return {
		imageProductIdx: index("imageProduct_idx").using("btree", table.productId.asc().nullsLast()),
...
(table) => {
	return {
		productNameIdx: index("ProductName_idx").using("btree", table.name.asc().nullsLast()),
...
(table) => {
	return {
		productIdx: index("product_idx").using("btree", table.productId.asc().nullsLast()),
...
(table) => {
	return {
		orderMadeIdx: index("order_made_idx").using("btree", table.orderId.asc().nullsLast()),
		productOrderedIdx: index("product_ordered_idx").using("btree", table.productId.asc().nullsLast()),
